...
> **开放测试 key（免注册体验）。** 运营方可能提供一把**开放 key**（同样形如 `sk-...`），用法和普通 key 完全一样（放进 `OFFERDAO_API_KEY`、带上鉴权头），区别是它**不绑定任何账号**，用户**无需注册 / 登录**即可直接搜索 / 发布、快速上手。注意：
> - 用开放 key **发布的岗位不归属任何人**，因此**不会出现在「我的发布」里**（但照常进审核队列，通过后才公开）；
> - 开放 key **仅供测试 / 体验**，可能设有**有效期**与**每日调用上限**；
> - 一旦开放 key **失效 / 过期（`401`）或超出当日上限（`429`）**，请改用**个人 API key**——按上面「如何获取 API key」登录后在「我的设置 → API keys」自助创建；换成个人 key 后，发布的岗位会归属到本人，可在登录态「我的发布」里查看 / 管理。
